...
Arbol* insertarUsuario_ABO(Arbol* arbol, char* nombreUsuario, char* telefonoUsuario)
{
	...
	if (hijo_IZQUIERDO(arbol) == NULL && strcmp(obtenerUsuarioArbol(hoja),obtenerUsuarioArbol(arbol)) < 0)
	{
		arbol->hijoIzquierdo = hoja;
		return arbol;
	}
	if (hijo_DERECHO(arbol) == NULL && strcmp(nombreUsuario,obtenerUsuarioArbol(arbol)) >= 0)
	{
	...
	{
		arbol->hijoIzquierdo = insertarUsuario_ABO(hijo_IZQUIERDO(arbol), nombreUsuario, telefonoUsuario);
		return arbol;
	}
	else
	{
		arbol->hijoDerecho = insertarUsuario_ABO(hijo_DERECHO(arbol), nombreUsuario, telefonoUsuario);
		return arbol;
	}
	return arbol;
}
...
Arbol* buscarUsuario(Arbol* arbol, char* nombreUsuario)
{
	...
	while (indice != NULL)
	{
		char* usuarioActual = obtenerUsuarioArbol(indice);
