...
/**
	@brief Funcion que inserta un elemento en la lista telefonos.
	@param L es una Lista de nodos.
	@param telefonoUsuario es el elemento a guardar en la lista.
	@returns Lista (puntero), con los datos actualizados.
*/
ListaTelefonos* InsertarTelefono(ListaTelefonos* L, int pos, char* telefonoUsuario)
{
	int i;
	// Si la pos ingresada es menor a 0, retorno la lista.
	if(pos < 0)
	...
	if(pos == 0)
	{
		ListaTelefonos* nueva = crearNodoTelefono();
		nueva->telefonoUsuario = telefonoUsuario;
		nueva->siguiente = L;
		return nueva;
	}
	i = 0;
	ListaTelefonos *indice = crearNodoTelefono();
	indice = L;
	// Muevo el puntero a la posicion - 1 que no sea nula.
	while (indice != NULL && i < pos -1)
	{
